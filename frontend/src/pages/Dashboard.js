...
function Dashboard({ user, onLogout }) {
    ...
    const fetchTanahList = async (userId) => {
        try {
            ...
            const response = await axios.get(`http://localhost:8080/api/tanah?userId=${userId}`);
    ...
    const fetchLahanByTanah = async (tanahId) => {
        try {
            const response = await axios.get(`http://localhost:8080/api/lahan/by-tanah/${tanahId}`);
            const data = Array.isArray(response.data) ? response.data : [];
            setLahanList(data);
            setSelectedLahan('');
        } catch (err) {
    ...
    const handleOkClick = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:8080/api/sensors/history/${selectedLahan}`);
    ...
    const handleTriggerRead = async () => {
        try {
            setLoading(true);
            await axios.post('http://localhost:8080/api/sensors/trigger-read');
        ...
        <div className="dashboard-container">
            <header className="dashboard-header">
                ...
                <div className="header-right">
                    <span className="user-info">Selamat datang, <strong>{user?.username}</strong></span>
                    ...
                    <div className="filter-controls">
                        <div className="filter-group">
                            ...
                            >
                                ...
                                {Array.isArray(tanahList) && tanahList.map((tanah) => (
                                    <option key={tanah.idTanah} value={tanah.idTanah}>
                        ...
                        <div className="filter-group">
                            ...
                            >
                                ...
                                {Array.isArray(lahanList) && lahanList.map((lahan) => (
                                    <option key={lahan.idLahan} value={lahan.idLahan}>
                                        {lahan.note}
                                    </option>
                                ))}
                            </select>
